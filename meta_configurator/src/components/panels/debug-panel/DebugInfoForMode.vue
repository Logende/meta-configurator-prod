...
<template>
    <div class="flex-grow overflow-y-auto">
      <div>
        <h1>Session Mode: {{ props.sessionMode }}</h1>
        <p>Session currentPath: {{ session.currentPath.value }}</p>
        <p>Session currentSelectedElement: {{ session.currentSelectedElement.value }}</p>
        <Divider />
      </div>
    </div>
</template>
