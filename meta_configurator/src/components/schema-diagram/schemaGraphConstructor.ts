...
function constructObjectNode(absolutePath: Path, name: string, schema: JsonSchemaObjectType, graph: SchemaGraph) {
    ...
    if (schema.allOf) {
        constructObjectSubSchemasContent(absolutePath, [...absolutePath, "allOf"], schema.allOf, graph, EdgeType.ALL_OF)
    }
    if (schema.anyOf) {
        constructObjectSubSchemasContent(absolutePath, [...absolutePath, "anyOf"], schema.anyOf, graph, EdgeType.ANY_OF)
    }
    if (schema.oneOf) {
        constructObjectSubSchemasContent(absolutePath, [...absolutePath, "oneOf"], schema.oneOf, graph, EdgeType.ONE_OF)
    }
...
function constructObjectSubSchemasContent(nodePath: Path, absolutePath: Path, subSchemas: JsonSchemaType[], graph: SchemaGraph, edgeType: EdgeType) {
...
function constructObjectSubSchemaContent(nodePath: Path, absolutePath: Path, subSchema: JsonSchemaType, graph: SchemaGraph, edgeType: EdgeType) {
        if (typeof subSchema === 'object') {
            if (subSchema.$ref) {
                graph.edges.push(new EdgeData(
                    nodePath,
                    jsonPointerToPath(subSchema.$ref.replace("\#", "")),
                    edgeType
                ));
            } else {
                ...
                constructObjectNode(
                    absolutePath,
                    subObjectNodeId,
                    subSchema as JsonSchemaObjectType,
                    graph
                );
                graph.edges.push(new EdgeData(
                    nodePath,
...
function constructObjectAttribute(nodePath: Path, absolutePath: Path, name: string, schema: JsonSchemaObjectType, required: boolean, graph: SchemaGraph): SchemaObjectAttributeData {
    ...
    if (schema.type == 'object') {
        ...
        constructObjectSubSchemaContent(nodePath, [...absolutePath, "items"], schema.items as JsonSchemaType, graph, EdgeType.ARRAY_ATTRIBUTE)
