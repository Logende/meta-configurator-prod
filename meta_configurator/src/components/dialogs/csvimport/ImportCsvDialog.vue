...
<template>
  <Dialog v-model:visible="showDialog" header="Import CSV">
    ...
    <div class="flex flex-wrap justify-content-center gap-3 bigger-dialog-content">
      ...
      <p>The import will insert the CSV data into your current document, overwriting existing data in case of a conflict.</p>
      ...
      <Button label="Select CSV Document" @click="requestUploadFile" class="p-button-raised p-button-rounded"></Button>
      <FontAwesomeIcon v-if="currentUserCsv.length > 0" icon="fa-regular fa-circle-check" class="text-green-500 ml-2" />
    </div>



      <div v-if="currentUserCsv.length > 0" class="flex flex-wrap justify-content-center gap-3 bigger-dialog-content">

        <Divider />


        <p>CSV file has {{ currentUserCsv.length }} rows and {{ currentColumnMapping.length }} columns
          (attributes).</p>
        <p>Define the mapping from the CSV to the JSON document for each attribute.</p>

      <div>
        <CsvImportColumnMapping v-for="column in currentColumnMapping" :key="column.index" :column-data="column" />
      </div>

      <Divider />


        <Button @click="submitImport" class="p-button-raised p-button-rounded" label="Import"></Button>
      </div>
    </div>
  </Dialog>
</template>
