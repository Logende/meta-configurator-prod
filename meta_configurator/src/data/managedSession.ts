...
public isExpanded(path: Path): boolean {
  ...
  public expand(path: Path): void {
    ...
    this._currentExpandedElements.value = {...this._currentExpandedElements.value, [pathAsString]: true};
  }
...
public collapse(path: Path): void {
  ...
  public isNodeHighlighted(node: ConfigDataTreeNode) {
    return this.currentSearchResults.value.map(searchResult => searchResult.path)
